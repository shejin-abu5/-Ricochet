...
export function LoginForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex w-full max-w-sm flex-col gap-4">
      ...
      {/* The "Don't have an account? Sign up" link used to live here. It moved
          into the Log in / Sign up toggle at the top of the column, so there's
          now exactly ONE control that switches modes instead of two that do
          the same thing in different visual languages. */}
    </form>
  )
}
