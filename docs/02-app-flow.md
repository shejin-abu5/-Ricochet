...
Each flow below lists: entry point → steps → screens → what state is involved (server state via TanStack Query vs global client state via Zustand vs local state), so the architecture decisions are visible in context.
...
2. Filters change → triggers a new `useQuery(['matches', filters])` — changing filters changes the **query key**, which changes the cache entry
...
**State:** Optimistic update via TanStack Query — `onMutate` applies the change immediately, `onError` rolls the cache back, `onSettled` refetches to reconcile.

## 5. Team creation & management flow

1. `/teams/new` — name, logo upload, home location → `useMutation(createTeam)` → creator is assigned the captain role
...
This is the most architecturally involved flow: ephemeral auction state in Zustand, reconciled against the server on settle.
...
- Unread count badge in the nav = derived from that same query's data — **do not duplicate server data into Zustand just to show a badge**; select/derive it instead
