# Product Requirements Document — Ricochet

## 1. Overview

Ricochet is a football (soccer) match-organizing platform, inspired by Playo. Players find and join pickup matches near them. Beyond the core Playo flow, Ricochet adds a competitive layer: registered teams, a player transfer/bidding market, and organizer-run tournaments.
...
- Real-money bidding — the player market is a virtual/points-based mechanic, not real currency
...
- **Scalability:** match/team data is paginated and query-cached; bidding uses optimistic concurrency to handle race conditions on simultaneous bids.
